...
function renderWizLigaPaso(paso) {
  ...
  const esUltimo = paso === _WIZ_LIGA_TOTAL;
  if (btnNext) {
    btnNext.innerHTML = esUltimo
      ? 'Crear liga 🛼'
      : 'Continuar <span class="material-icons" style="font-size:18px;">arrow_forward</span>';
  }

  const OPCIONALES = [2,4,5,6,7,8,9,10];
  const esOpcional = OPCIONALES.includes(paso);

  function html(emoji, titulo, desc, contenidoHTML) {
    return `
      <span class="wiz-eq-emoji">${emoji}</span>
      <h2 class="wiz-eq-title">${titulo}</h2>
      <p class="wiz-eq-desc">${desc}${esOpcional ? ' <span class="wiz-optional-badge">Opcional</span>' : ''}</p>
      <div class="wiz-eq-content">${contenidoHTML}</div>
    `;
  }
  ...
  if (paso === 0) {
    contenido.innerHTML = `
      <span class="wiz-eq-emoji">👋</span>
      <h2 class="wiz-eq-title">Primero, identifícate</h2>
      <p class="wiz-eq-desc">Inicia sesión con tu cuenta de Google para continuar.</p>
      <div class="wiz-eq-content">
  ...
  if (paso === 1) {
    contenido.innerHTML = html('🏟️', '¿Cómo se llama tu liga?', 'El nombre de la organización.', `
      <input id="wiz-liga-nombre" type="text" placeholder="Nombre de la liga"
        class="wiz-eq-input" value="${_wizLiga.nombreLiga}"
        oninput="_wizLiga.nombreLiga=this.value"
        onkeydown="if(event.key==='Enter') wizLigaPasoSiguiente()">
    `);
    setTimeout(() => document.getElementById('wiz-liga-nombre') && document.getElementById('wiz-liga-nombre').focus(), 100);
  }

  if (paso === 2) {
    const preview = _wizLiga.ligaImagenBase64
      ? `<img src="${_wizLiga.ligaImagenBase64}" class="wiz-liga-logo-img">`
      : `<span class="material-icons wiz-liga-logo-ph">add_photo_alternate</span>`;
    contenido.innerHTML = html('🖼️', 'Logo de la liga', 'Sube un logo o ícono que represente a tu liga.', `
      <label class="wiz-liga-logo-label" id="wiz-liga-img-label">
        ${preview}
  ...
  if (paso === 3) {
    contenido.innerHTML = html('🛼', '¿Cómo se llama tu equipo?', 'Puede tener el mismo nombre que la liga. Puedes agregar más equipos desde ajustes.', `
      <input id="wiz-liga-equipo-nombre" type="text" placeholder="Nombre del equipo"
        class="wiz-eq-input" value="${_wizLiga.nombreEquipo}"
        oninput="_wizLiga.nombreEquipo=this.value"
        onkeydown="if(event.key==='Enter') wizLigaPasoSiguiente()">
    `);
    setTimeout(() => document.getElementById('wiz-liga-equipo-nombre') && document.getElementById('wiz-liga-equipo-nombre').focus(), 100);
  }

  if (paso === 4) {
    contenido.innerHTML = html('🏆', '¿Qué categoría es tu equipo?', 'Selecciona la categoría en la que compite.', `
      <div class="wiz-liga-cat-row">
        ${['A','B','C'].map(cat => `
          <button onclick="seleccionarCategoriaLigaWiz('${cat}')" id="wiz-liga-cat-${cat}"
            class="wiz-liga-cat-btn ${_wizLiga.categoria === cat ? 'wiz-liga-cat-btn--activo' : ''}">${cat}</button>
        `).join('')}
      </div>
    `);
  }
  ...
  if (paso === 5) {
    const preview = _wizLiga.logoBase64
      ? `<img src="${_wizLiga.logoBase64}" class="wiz-liga-logo-img">`
      : `<span class="material-icons wiz-liga-logo-ph">add_photo_alternate</span>`;
    contenido.innerHTML = html('🎨', 'Logo del equipo', 'Sube el logo de tu equipo. Puedes cambiarlo desde ajustes.', `
      <label class="wiz-liga-logo-label" id="wiz-liga-logo-label">
        ${preview}
  ...
  if (paso === 6) {
    contenido.innerHTML = html('🌎', '¿De qué país es tu liga?', 'Nos ayuda a construir una red de ligas por región.', `
      <div class="wiz-eq-content" style="position:relative;">
        <input id="wiz-liga-pais-input" type="text" placeholder="Buscar país…"
          class="wiz-eq-input" autocomplete="off" value="${_wizLiga.pais}"
          oninput="filtrarPaisesLiga(this.value)">
        <div id="wiz-liga-pais-lista" class="wiz-liga-pais-lista" style="display:none;"></div>
      </div>
    `);
    setTimeout(() => document.getElementById('wiz-liga-pais-input') && document.getElementById('wiz-liga-pais-input').focus(), 100);
  }

  if (paso === 7) {
    contenido.innerHTML = html('🏙️', '¿De qué ciudad?', 'La ciudad donde opera principalmente tu liga.', `
      <input id="wiz-liga-ciudad" type="text" placeholder="Ciudad"
        class="wiz-eq-input" value="${_wizLiga.ciudad}"
        oninput="_wizLiga.ciudad=this.value"
        onkeydown="if(event.key==='Enter') wizLigaPasoSiguiente()">
    `);
    setTimeout(() => document.getElementById('wiz-liga-ciudad') && document.getElementById('wiz-liga-ciudad').focus(), 100);
  }

  if (paso === 8) {
    contenido.innerHTML = html('📅', '¿En qué año se fundó tu liga?', 'El año de fundación de la liga.', `
      <input id="wiz-liga-anio" type="number" placeholder="Ej: 2018"
        class="wiz-eq-input" value="${_wizLiga.anioFundacion}"
        min="1990" max="${new Date().getFullYear()}"
        oninput="_wizLiga.anioFundacion=this.value"
        onkeydown="if(event.key==='Enter') wizLigaPasoSiguiente()">
    `);
    setTimeout(() => document.getElementById('wiz-liga-anio') && document.getElementById('wiz-liga-anio').focus(), 100);
  }

  if (paso === 9) {
    contenido.innerHTML = html('📖', 'Cuéntanos sobre tu liga', 'Su misión, origen e ideales. Aparecerá en el directorio de ligas.', `
      <div class="wiz-step0-note">
        <span>🌐</span>
        <span>Esta información es parte del <strong>Directorio Global de Ligas de Roller Derby</strong> que estamos construyendo para conectar ligas de todo el mundo.</span>
      </div>
      <textarea id="wiz-liga-descripcion" placeholder="Ej: Somos una liga inclusiva fundada en 2018…"
        class="wiz-eq-input" rows="4" maxlength="500"
        oninput="_wizLiga.descripcion=this.value"
        style="resize:none;line-height:1.5;">${_wizLiga.descripcion}</textarea>
    `);
    setTimeout(() => document.getElementById('wiz-liga-descripcion') && document.getElementById('wiz-liga-descripcion').focus(), 100);
  }

  if (paso === 10) {
    const esNumero = _wizLiga.contacto && !_wizLiga.contacto.startsWith('@') && !_wizLiga.contacto.startsWith('http');
    contenido.innerHTML = html('📬', '¿Cómo pueden contactarte?', 'Deja un Instagram o un número de WhatsApp.', `
      <div class="wiz-liga-tipo-row">
        <button id="wiz-liga-tipo-social" onclick="toggleTipoContactoLiga('social')"
          class="wiz-liga-tipo-btn ${!esNumero ? 'wiz-liga-tipo-btn--activo' : ''}">
