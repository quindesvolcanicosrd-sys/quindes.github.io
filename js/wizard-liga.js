...
function bindImageInput(opts) {
  ...
  const showPreview = function(base64) {
    ...
    if (overlay) { overlay.classList.remove('wiz-hidden'); overlay.classList.add('reg-avatar-overlay--badge'); }
...
function wizLigaPasoSiguiente() {
  ...
  if (_wizLigaPaso === 3) {
    ...
    if (customVacio) {
      mostrarDialogConfirm('No has escrito el nombre de tu ciudad. ¿Deseás continuar?', function() { renderWizLigaPaso(_wizLigaPaso + 1); });
      return;
    }
    if (_wizLiga.pais && !_wizLiga.ciudad) {
      mostrarDialogConfirm('No has seleccionado ciudad. ¿Deseás continuar?', function() { renderWizLigaPaso(_wizLigaPaso + 1); });
      return;
    }
  }
...
function wizLigaRecibirImagenRecortada(stateKey, base64DataUrl) {
  ...
  var overlayMap = {
    'ligaImagenBase64': 'wiz-liga-img-overlay',
    'logoBase64':       'wiz-liga-logo-overlay',
    'fotoBase64':       'wiz-liga-foto-overlay',
  };
  var overlayEl = document.getElementById(overlayMap[stateKey]);
  if (overlayEl) {
