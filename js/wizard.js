// ============================================================
//  PIVOT APP — wizard.js
//  Wizard de registro + Wizard de liga (unificado)
// ============================================================

// ── UTILIDAD: procesado de imagen ────────────────────────────
...
function bindImageInput({ inputId, previewId, placeholderId, stateKey, config = {} }) {
  ...
  dropZone.addEventListener('dragover',  e => { e.preventDefault(); dropZone.classList.add('wiz-drag-over'); });
  dropZone.addEventListener('dragleave', ()  => dropZone.classList.remove('wiz-drag-over'));
...
function createWizard(config) {
  ...
  function resolveNext(stepConfig, idx) {
    let nextStep;
    if (typeof stepConfig?.next === 'function') nextStep = stepConfig.next(regData);
    else if (stepConfig?.next)                  nextStep = stepConfig.next;
  ...
  function back() {
    ...
    const stepConfig = config.steps[currentStep];
    let prevStep;
    if (typeof stepConfig?.back === 'function') prevStep = stepConfig.back(regData);
    else if (stepConfig?.back)                  prevStep = stepConfig.back;
...
// ── CONFIGURACIÓN DE PASOS DEL WIZARD REGISTRO ───────────────
...
const WIZ_STEPS = {
  ...
  '4': {
    label: 'País',
    validate: () => (!regData.pais ? 'Selecciona tu país de origen 🌎' : null)
  },
  ...
  '8': {
    label: 'Rol',
    validate: () => (!regData.rolJugadorx ? 'Selecciona tu rol en el equipo 🏅' : null)
  },

  '9': {
    label: 'Asistencia',
    validate: () => (!regData.asisteSemana ? 'Indica cuántas veces entrenas por semana 🏋️' : null)
  },
...
function mostrarRegistroWizard() {
  ...
  registroWizard = createWizard({
    ...
    transition:  (from, to, fwd) => wizGoTo(to, fwd),
    onAfterChange: (step) => {
      wizStep = step;
      wizUpdateHeader();
      wizSaveDraft();
  ...
  fetch('https://ipapi.co/json/')
    ...
    .then(data => {
      if (!regData.codigoPais) {
        const match = REG_CODIGOS.find(c => c.includes(data.country_calling_code));
        if (match) {
  ...
  if (saved) {
    ...
    catch { console.warn('Error cargando draft'); }
  } else {
  ...
  document.querySelectorAll('#wiz-viewport .wiz-step').forEach(s => {
...
function wizIntroStart() {
  ...
  setTimeout(() => {
    ...
    if (s1) {
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => s1.classList.add('wiz-animate')));
    }
  }, 200);
}

// ── TRANSICIÓN DE PASOS REGISTRO ─────────────────────────────
...
function wizUpdateHeader() {
  ...
  const stepName = WIZ_STEPS_CONFIG?.[wizStep]?.label || '';
...
function wizLiveValidate({ value, validator, onValid, onInvalid }) {
...
function initRegistroListeners() {
  ...
  document.addEventListener('click', e => {
    ...
    if (e.target.id === 'reg-pais-btn' || e.target.closest('#reg-pais-btn')) {
      abrirBottomSheet('País', REG_PAISES, regData.pais || '', val => {
        regData.pais = val;
...
function renderWizLigaPaso(paso) {
  ...
  if (paso === 2) {
    wizLigaGoTo(el => {
      ...
      if (input) {
        ...
        input.addEventListener('input', e => _wizLiga.nombreLiga = e.target.value);
        input.addEventListener('keydown', e => { if (e.key === 'Enter') wizLigaPasoSiguiente(); });
  ...
  if (paso === 4) {
    wizLigaGoTo(el => {
      ...
      function cargarCiudades(pais) {
        ...
        if (esCustom) { selCiudad.value = '__otro__'; wrapCustom.classList.remove('wiz-hidden'); inputCustom.value = _wizLiga.ciudad; }
        else           wrapCustom.classList.add('wiz-hidden');
      }
      ...
      inputCustom.addEventListener('input', e => _wizLiga.ciudad = e.target.value);
    }, forward);
    return;
  }
  ...
  if (paso === 5) {
    wizLigaGoTo(el => {
      ...
      if (inputAnio) {
        ...
        inputAnio.addEventListener('input', e => _wizLiga.anioFundacion = e.target.value);
      }
      if (inputDesc) {
        inputDesc.value = _wizLiga.descripcion || '';
        inputDesc.addEventListener('input', e => _wizLiga.descripcion = e.target.value);
      }
  ...
  if (paso === 6) {
    wizLigaGoTo(el => {
      ...
      if (input) {
        ...
        input.addEventListener('input', e => _wizLiga.contactoSocial = e.target.value);
        setTimeout(() => input.focus(), 350);
      }
    }, forward);
    return;
  ...
  if (paso === 7) {
    wizLigaGoTo(el => {
      ...
      if (input) {
        ...
        input.addEventListener('input', e => _wizLiga.nombreEquipo = e.target.value);
        input.addEventListener('keydown', e => { if (e.key === 'Enter') wizLigaPasoSiguiente(); });
        setTimeout(() => input.focus(), 350);
      }
    }, forward);
    ...
    wizLigaGoTo(el => {
      ...
      ['A','B','C'].forEach(cat => {
        ...
        btn.addEventListener('click', () => {
          ...
          [...wrap.children].forEach(b => { b.classList.remove('chip-active'); b.classList.add('chip-inactive'); });
  ...
  if (paso === 10) {
    wizLigaGoTo(el => {
      ...
      document.getElementById('wiz-liga-skip-10')?.addEventListener('click', wizLigaPasoSiguiente);
    }, forward);
    return;
  }
  ...
  if (paso === 11) {
    wizLigaGoTo(el => {
      ...
      input.addEventListener('input', e => _wizLiga.nombre = e.target.value);
      input.addEventListener('keydown', e => { if (e.key === 'Enter') wizLigaPasoSiguiente(); });
    ...
    wizLigaGoTo(el => {
      ...
      document.getElementById('wiz-liga-perfil-pais-btn').onclick = () => {
        abrirBottomSheet('Nacionalidad', REG_PAISES, _wizLiga.paisPerfil || '', val => {
          _wizLiga.paisPerfil = val; display.textContent = val;
        });
      };
    }, forward);
    return;
    ...
    wizLigaGoTo(el => {
      ...
      document.getElementById('wiz-liga-perfil-codigo-btn').onclick = () => {
        abrirBottomSheet('Código', REG_CODIGOS, _wizLiga.codigoPais || '', val => {
          _wizLiga.codigoPais = val; display.textContent = val;
        });
      };
    }, forward);
    return;
    ...
    wizLigaGoTo(el => {
      ...
      document.getElementById('wiz-liga-perfil-fecha-btn').onclick = () => {
        abrirDatePicker(_wizLiga.fechaNacimiento || '', val => {
          _wizLiga.fechaNacimiento = val; display.textContent = val;
        });
      };
  ...
  if (paso === 20) {
    wizLigaGoTo(el => {
      cloneTpl('tpl-wiz-liga-20', el);
      document.getElementById('wiz-liga-perfil-emergencia').oninput = e => _wizLiga.contactoEmergencia = e.target.value;
    }, forward);
    return;
  }
}

// ── VALIDACIÓN Y NAVEGACIÓN LIGA ──────────────────────────────
function wizLigaPasoSiguiente() {
  if (_wizLigaPaso === 2  && !_wizLiga.nombreLiga.trim())    { mostrarToastGuardado('⚠️ Escribe el nombre de la liga'); return; }
  ...
  if (_wizLigaPaso === _WIZ_LIGA_TOTAL) { wizLigaSubmit(); return; }
